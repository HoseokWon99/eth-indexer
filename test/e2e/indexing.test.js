...
describe('Indexing Tests', async () => {
  ...
  it('Transfer tx hashes are indexed', () => {
  ...
  it('Approval tx hashes are indexed', () => {
  ...
  it('Transfer count is at least 122 (120 txs + 2 constructor mints)', () => {
    const transferRows = dbRows.filter(r => r.event_name === 'Transfer');
    expect(transferRows.length).toBeGreaterThanOrEqual(122);
  });

  it('Approval count is exactly 120', () => {
    const approvalRows = dbRows.filter(r => r.event_name === 'Approval');
